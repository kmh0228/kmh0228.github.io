...
import useMessage from '@/hooks/useMessage/useMessage.ts'
...
const addFavorite = (title: string, url: string) => {
  try {
    ...
    try {
      ...
      message.warning('抱歉，您所使用的浏览器无法完成此操作。加入收藏失败，请使用Ctrl+D进行添加')
    }
  }
}
 ...
 * @param filename  文档名称
 */
const exportTextAsFile = (text: string, filename: string) => {
  // 创建一个Blob实例，类型为纯文本
...
export {
  ...
  fetDownload, // 通过 blob下载
  copy, // 复制
  exportTextAsFile, // 把文字导出成.txt
  importFile, // 导入文件
  readTxtFile // 读取text文件内容
}
